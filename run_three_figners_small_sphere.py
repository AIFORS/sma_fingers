...
parser = argparse.ArgumentParser(description="Run ThreeFingers environment with multi-finger reference tracking from Dec25_small.csv.")
...
def main():
    ...
    times, refs_real, refs_norm, real_data, real_norm, baselines, force_refs, force_real = load_reference_sequences(
        Path("finger_open_close/Dec25_small.csv"), args_cli.sampling_time, experiments
    )

    log_path = Path("outputs/three_figners_small_sphere_tracking.csv")
    ...
    if args_cli.video:
        video_folder = Path("outputs/videos/three_figners_small_sphere")
    ...
    try:
        ...
        plot_tracking(
            Path("outputs/three_figners_small_sphere_tracking.png"),
            times,
            refs_real,
            sim_denorm,
            real_data,
